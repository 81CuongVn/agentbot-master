...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        const embed = new MessageEmbed()
            ...
            .addField('Chung', stripIndent`**--> Tên server:** ${message.guild.name}
        ...
        **--> Mức độ xác minh:** ${serverFlags.verificationLevels[message.guild.verificationLevel]}`)
            .addField('Thống kê', stripIndent`**--> Số role:** ${roles.length}
        ...
        **--> Ngày tạo server:** ${moment(message.guild.createdTimestamp).format('MM/DD/YYYY hh:mm:ss')}
        `)
            .addField('Thống kê thành viên', stripIndent`**--> Online:** ${members.filter(m => m.presence.status === 'online').size} 
